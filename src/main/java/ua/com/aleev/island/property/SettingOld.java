...
public class SettingOld {
    ...
    public static final int PLANT_MAX_COUNT = 200;
    ...
    static {
        for (int i = 0, n = AnnimalsRationOld.names.length; i < n; i++) {
            String animalKey = AnnimalsRationOld.names[i];
            rationMap.putIfAbsent(animalKey, new LinkedHashMap<>());
            for (int j = 0; j < n; j++) {
                int ration = AnnimalsRationOld.setProbablyTable[i][j];
                if (ration > 0) {
                    rationMap.get(animalKey).put(AnnimalsRationOld.names[j], ration);
                }
            }
        }
    }
